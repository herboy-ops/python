...
def read_file(file_path):
    ...
    try:
        return (pd.read_excel(file_path) if ext in ['xlsx', 'xls'] 
                else pd.read_csv(file_path, delimiter=('\t' if ext == 'txt' else ',')))
...
def process_files(file1_path, file2_path, payment_type):
    ...
    df1.columns, df2.columns = df1.columns.str.strip().str.lower(), df2.columns.str.strip().str.lower()
    ...
    if df1_col in df1.columns and df2_col in df2.columns:
        try:
            ...
            unmatched_1, unmatched_2 = df1[~df1[df1_col].isin(matched[df1_col])], df2[~df2[df2_col].isin(matched[df2_col])]
            summary = {
...
def create_summary_chart(payment_type, summary):
    ...
    counts = [summary[key] for key in summary]
    ...
    plt.pie(counts, labels=[f'{cat}\n{count:,}' for cat, count in zip(categories, counts)],
            colors=['#ff9999', '#66b3ff', '#99ff99', '#ffcc99', '#c2c2f0'], startangle=90, counterclock=False, 
